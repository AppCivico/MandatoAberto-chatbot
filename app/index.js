...
bot.onEvent(async (context) => {
	...
	if (context.event.isPostback && (context.state.dialog === 'prompt' || context.event.postback.payload === 'greetings')) {
		...
		if (context.event.message) {
			context.event.message.text = ''; // what's the point of this
		}
	}
	...
	case 'listening':
		...
		timer = setTimeout(async () => {
			await context.setState({ sendIntro: true });
			const issueCreatedMessage = await MandatoAbertoAPI.getAnswer(context.state.politicianData.user_id, 'issue_created');
			let endMessage;
			if (issueCreatedMessage.content) {
				endMessage = `${issueCreatedMessage.content}\nVocê terminou de escrever sua mensagem?`;
			} else {
	...
	case 'issue_created': {
		const issueCreatedMessage = await MandatoAbertoAPI.getAnswer(context.state.politicianData.user_id, 'issue_created');
		await context.sendButtonTemplate(issueCreatedMessage.content, [opt.backToBeginning]);
		await context.setState({ dialog: 'prompt' });
		break; }
	}
});
